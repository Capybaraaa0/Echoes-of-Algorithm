...
(function() {
    ...
    var originalIndex = textState.index;
    if (c === '\x1b' && this.obtainEscapeCode(textState) === 'I') {
        textNextX = textState.x + Window_Base._iconWidth;
    } else {
        textNextX = textState.x + this.textWidth(c);
    }
    textState.index = originalIndex;

    if (textNextX > maxWidth) {
        // --- 横排文本的智能换行逻辑 ---
        if (!paramVerticalWriting) {
            var text = textState.text;
            var lastDrawnCharIndex = textState.index - 1;

            // 如果刚刚绘制的字符本身就是空格或换行符，则直接使用旧逻辑换行。
            if (text[lastDrawnCharIndex].match(/[\s\n]/)) {
                textState.index--;
            ...
            // 通过反向查找，定位当前单词的起始位置。
            var wordStartIndex = textState.index - 1;
            while (wordStartIndex > 0 && !text[wordStartIndex - 1].match(/[\s\n\x1b]/)) {
                wordStartIndex--;
            }

            // 确认该单词不是在本行的开头（即单词本身没有长于整行）。
            var lineHeadIndex = text.lastIndexOf('\n', lastDrawnCharIndex) + 1;
            if (wordStartIndex > lineHeadIndex) {
                // 计算需要被清除并重绘的文本部分。
                var textToClear = text.substring(wordStartIndex, textState.index);
                var widthToClear = this.textWidth(textToClear);

                // 在画布上擦除这部分文本。
                this.contents.clearRect(textState.x - widthToClear, textState.y, widthToClear, this.lineHeight());

                // 将索引精确地设置回单词的开头。
                textState.index = wordStartIndex;
    ...
    // [通用辅助函数] - 用于获取单词和宽度
    Window_Base.prototype.nm_getWordAndWidth = function(textState) {
    var match = /^[^\s\n\x1b]+/.exec(textState.text.substring(textState.index));
    var word = match ? match[0] : '';
    return { word: word, width: this.textWidth(word) };
    };

    // [核心修复] - 重写插件的 NovelMessage 字符处理函数
    var _NobleMushroom_PNC_Alias = Window_NovelMessage.prototype.processNormalCharacter;
    Window_NovelMessage.prototype.processNormalCharacter = function(textState) {
    // 检查是否为横排文本且启用了自动换行
    var isVertical = typeof paramVerticalWriting !== 'undefined' && paramVerticalWriting;
    if (!isVertical && paramAutoWordWrap) {
        var c = textState.text[textState.index];
        if (c > ' ' && textState.x > 0) {
            var prevC = textState.text[textState.index - 1];
            if (prevC && prevC.match(/[\s\n]/)) {
                var wordData = this.nm_getWordAndWidth(textState);
                if (wordData.width > 0 && (this.contentsWidth() - textState.x < wordData.width)) {
                    // 1. 执行换行。
                    this.processNewLine(textState);

                    // 2.【关键修正】根据您的日志，我们知道上面的换行会错误地将索引+1。
                    //    我们在这里手动将其-1，以纠正这个错误。
                    textState.index--;

                    // 3. 立即返回，让游戏在下一帧以正确的索引处理首字母。
                    return;
                }
            }
        }
    ...
    // 如果不满足换行条件，则调用原始函数
    _NobleMushroom_PNC_Alias.call(this, textState);
    };

    // [兼容性修改] - 禁用旧的换行逻辑以避免冲突
    var _NobleMushroom_PAWW_Alias = Window_NovelMessage.prototype.processAutoWordWrap;
    Window_NovelMessage.prototype.processAutoWordWrap = function(textState) {
    var isVertical = typeof paramVerticalWriting !== 'undefined' && paramVerticalWriting;
    if (isVertical) {
        // 仅为纵排文本保留旧逻辑
        _NobleMushroom_PAWW_Alias.call(this, textState);
    }
    // 对于横排文本，此函数不执行任何操作。
    };
})();
